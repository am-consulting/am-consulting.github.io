fun.read.ilbs.icbs <- function(path.to.folder = '/Desktop/R_Data_Write/'){
  ...
  # required package:XLConnect,Nippon
  buf.wd <- getwd()
  ...
  for(iii in seq(length(region.col.s))){
    ...
    region.col.ss <- which(!is.na(buf1[5,]))
    region.col.ee <- c(region.col.ss[-1]-1,ncol(buf1))
    for(iiii in seq(length(region.col.ss))){
      buf2 <- buf1[,c(1,2,region.col.ss[iiii]:region.col.ee[iiii])]
      ...
      buf3[,1] <- sapply(paste0(buf3[,1],buf3[,2]),zen2han)
      buf4 <- buf3[,-2]
