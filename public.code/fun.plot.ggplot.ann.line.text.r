fun.plot.ggplot.ann.rect <- function(g=g,xmin,xmax,ymin=-Inf,ymax=Inf,alpha=0.1,fill='red'){
  g <- g + annotate('rect',xmin = xmin,xmax = xmax,ymin = ymin,ymax = ymax,alpha = alpha,fill = fill)
  return(g)
}

fun.plot.ggplot.ann.text <- function(g=g,x,y=Inf,label,angle=0,vjust=1,hjust=0,size=7,col='#696969',family='Meiryo'){
  g <- g + annotate('text',x = x,y = y,label = label,angle = angle,vjust = vjust,hjust = hjust,size = size,col = col,family = family)
  return(g)
}

fun.plot.ggplot.vline <- function(g=g,xintercept,col='#696969',linetype=2){
  g <- g + geom_vline(xintercept = xintercept,col = col,linetype = linetype)
  return(g)
}

fun.plot.ggplot.hline <- function(g=g,yintercept,col='#696969',linetype=2){
  g <- g + geom_hline(yintercept = yintercept,col = col,linetype = linetype)
  return(g)
}

fun.plot.ggplot.text.repel <- function(g=g,data,size=6,load.library=F,col='#696969'){
...
fun.plot.ggplot.encircle <- function(g=g,data,size=1,expand=0.1,color='red',legend=F){
  g <- g + geom_encircle(data = data,aes(x = data$x, y = data$y),show.legend = legend,
                         color = color,size = size,expand = expand)
  return(g)
}

fun.plot.ggplot.mean.median.hline <- function(g=g,obj,date.col=1,data.col=2,col='#696969',linetype=2,angle=0,vjust=0,hjust=0,size=4){
