...
funPlotTimeSeries <- function() {
  ...
  # 民主党パート
  rectDateL <- max(as.numeric(as.Date('2009-09-16')),as.numeric(dataSet[1, 1]))
  rectDateR <- as.numeric(as.Date('2012-12-26'))
  if(as.numeric(dataSet[1,1]) < rectDateR){
    rectText <- '民主党政権'
...
needCCF <- 1
need1stdiff <- 0
par(mfrow = c(1, 2), family = 'Meiryo', mar = c(3, 3, 4, 2), oma = c(0, 0, 1, 0))
funPlotTimeSeries()
dataSet <- dataSetSA
funPlotTimeSeries()
