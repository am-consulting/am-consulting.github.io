...
#### Interest-rate difference between DGS and JGB

```{r warning=F,error=F,message=F,echo=F,results='asis'}
for(iii in 1:length(diffData)){
  cat(paste0('<b>',colnames(diffData[[iii]])[2],'</b>'))
  cat('<br>')
  cat(paste0(range(diffData[[iii]][,1]),collapse = ' ~ '))
  fun_markdownTable(obj = rbind(summary(diffData[[iii]][,2])))
  cat('<br>')
}
```
...
library(quantmod)
buf <- na.omit(getSymbols(Symbols = 'DEXJPUS',src = 'FRED',auto.assign = F))
buf <- data.frame(Date = index(buf),buf,stringsAsFactors = F,check.names = F,row.names = NULL)
colnames(buf)[2] <- 'USD/JPY'
mergeData <- merge(diffData[[6]],buf,by='Date')
colnames(mergeData)[2] <- 'Spread(%)'
fun_plotTimeSeries(obj = mergeData,lwdR = 1,dataSource = dataSource)
```

- 2016年大統領選挙以降

```{r warning=F,error=F,message=F,echo=F,fig.height=5}
fun_plotTimeSeries(obj = subset(mergeData,as.Date('2016-11-8')<=mergeData[,1]),lwdR = 1,dataSource = dataSource)
```
...
for(iii in 1:length(diffData)){
  tmpDF <-
    fun_transposeDataFrame(obj = diffData[[iii]],dateFormat = dateFormat,tailN = 4)
  if(iii == 1){
    recentDF <- tmpDF
  }else{
    recentDF <- rbind(recentDF,tmpDF)
  }
}
fun_markdownTable(obj = recentDF)
```
