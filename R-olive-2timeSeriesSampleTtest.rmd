...
dataTitle <- paste0('民主党政権時(Period1)と第二次安倍政権発足以降(Period2)の比較 - ',
                    '信用乗数(MB/広義流動性) 2016年8月'
                    )
dataSource <- '総務省'
...
dateRange02 <- c(as.Date('2013/1/1'), as.Date(tmp[nrow(tmp),1]))
```
...
comparisonData[1, ccc] <- paste(dateRange[[ccc]] , collapse = '~')
  ...
  comparisonData[13, ccc] <- format(adf.test(x)$p.value, digits = digitsN)
  comparisonData[14, ccc] <- format(adf.test(diff(x))$p.value, digits = digitsN)
}
comparisonData
...
# liner correlation
  resultlm <- lm(dataset[[iii]][, 2] ~ as.numeric(dataset[[iii]][, 1]))
  abline(resultlm,lwd=1)
# liner correlation
}
...
```{r,warning=F,error=F,message=F,results='hold',echo=F}
Date <- as.numeric(dataset[[1]][, 1])
Value <- as.numeric(dataset[[1]][, 2])
summary(lm(Value ~ Date))
```

```{r,warning=F,error=F,message=F,results='hold',echo=F}
Date <- as.numeric(dataset[[2]][, 1])
Value <- as.numeric(dataset[[2]][, 2])
summary(lm(Value ~ Date))
```
<hr>
```{r,warning=F,error=F,message=F,results='asis',echo=F}
