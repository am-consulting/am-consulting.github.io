...
dateFormat <- '%Y年%m月%d日'
dataTitle <-
  paste0('ゴールド価格(Source:World Gold Council)',' - ',format(tail(goldPriceData[, 1],1), dateFormat))
...
```{r warning=F,error=F,message=F,echo=F,results='hold'}
fun_plotTimeSeries(obj = goldPriceData,objX = 1,objYL = 2,objYR = 4,
                   lwdL = 1,lwdR = 1,dataTitle = paste0(dataTitle,'\n',title))
```
...
knitr::kable(tail(timeSeriesdata[,c(1,ccc)]),row.names = F)
cat('\n\n<hr>')
knitr::kable(tail(timeSeriesdataDiff[,c(1,ccc)]),row.names = F)
cat('\n\n<hr>')
knitr::kable(resultADF,row.names = T)
cat('\n\n<hr>')
knitr::kable(resultADFdiff,row.names = T)
cat('\n\n<hr>')
knitr::kable(statisticsTable,row.names = T)
cat('\n\n<hr>')
knitr::kable(statisticsTableDiff,row.names = T)
cat('\n\n<hr>')
knitr::kable(statisticsTableDiffMaxMin,row.names = T)
cat('\n\n<hr>')
knitr::kable(statisticsTableDiffRatio,row.names = T)
cat('\n\n<hr>')
knitr::kable(statSequencePositive,row.names = T)
cat('\n\n<hr>')
knitr::kable(statSequenceNegative,row.names = T)
cat('\n\n<hr>')
```
