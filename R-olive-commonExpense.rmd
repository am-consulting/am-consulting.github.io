...
library(beepr);library(dygraphs)
...
constructionType <- c(
  ...
  "橋梁保全工事",        #22
  "工事区分共通",        #23 一般管理費率
  "5億円以下:地方部",    #24 現場環境改善費(イメージアップ)
  "5億円以下:市街地"     #25 現場環境改善費(イメージアップ)
)
```

```{r warning=F, error=F, message=F, echo=F, results='hold'}
dataTitle <- 
  paste0('国土交通省土木工事積算基準等:', constructionType[constructionSegment])
```

***

- `r paste0("<h3><b>", dataTitle,"</b></h3>")`
...
par(mar = c(5,5,2,2),family = 'Meiryo',font.main = 1)
plot(
  obj[, 1],
  obj[, 2],
  type = 'S',
  ...
  xlab = colnames(obj)[1] ,
  ylab = colnames(obj)[2] ,
  main = paste0('対象額:',paste0(range(obj[,1]),'千円',collapse=' ~ ')),
  cex.axis = 1.0,
  cex.lab  = 1.0,
  cex.main = 1.0,
  lwd = 1
...
functionP <- 
  function(Pi,iA,iB){
    rateImageup <- iA*(Pi^iB)
    return(rateImageup)
  }
Pi <- seq(1000,500000000,by=1000)
colName <- c("現場環境改善費率(%)","現場環境改善費率(四捨五入前.%)","現場環境改善費率")
```

<h4><b>現場環境改善費率:対象額5億円以下:地方部</b></h4>
...
iB <- 
  -0.201
i <- 
  sapply(Pi,function(x) functionP(Pi = x,iA = iA,iB = iB))
rateAll[[constructionSegment]] <- 
  data.frame(Pi, round(i, 10), floor(100*as.numeric(i)+0.5)/100,check.names = F,stringsAsFactors = F) 
cat('- i = ',iA,' × Pi<sup>',iB,'</sup>')
fun_tableReshape()
fun_dataTable(obj = rateTable,
              orderColumn = 0,
              orderDirection = 'asc',
...
<h4><b>現場環境改善費率:対象額5億円以下:市街地</b></h4>
