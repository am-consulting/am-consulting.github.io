fun.import.jgb.interest.rate <- function(){
  ...
  for(iii in seq(length(csv.file))){
    ...
    data.set[[iii]] <-
      read.csv(paste0(base.url,csv.file[1]),header = T,skip = 1,stringsAsFactor = F,check.names = F,na.strings = '-')
  }
  if(colnames(data.set[[1]]) == colnames(data.set[[2]])){
    tmp <- rbind(data.set[[1]],data.set[[2]])
    tmp[,1] <- as.Date(tmp[,1])
    jgb.interest.rate <-
      data.frame(Date = tmp[,1],apply(tmp[,-1,drop = F],2,as.numeric),stringsAsFactors = F,check.names = F,row.names = NULL)
