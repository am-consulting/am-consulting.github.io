---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');htmlName<-'MovementOfFinancialIndicesBeforeJudgment Day';rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-',htmlName,'.html')) })
...
dataTitle <- 
  paste0("審判の日/リーマンショック前における金融指標の動き")
dataSource <- "FRED"
...
for(iii in seq(length(fredData))){
  obj <- 
    na.omit(subset(fredData[[iii]],startDate<=fredData[[iii]][,1] & fredData[[iii]][,1]<=JudgmentDay))
  fun_plotTimeSeries(obj = obj,chartType = 0,
                     dataSource = dataSource,mar = c(4,4,5,2),needLefLab = 0,fitLcolor = 2,
                     typeL = 'o',cexL = 1.2,fitlwdL = 2,
                     dataTitle = paste0('Movement from ',pastDays,' days before Judgment Day'))
  abline(v = as.numeric(JudgmentDay),col='red',lty=1)
  text(x = as.numeric(JudgmentDay),y = max(obj[,2]),
       labels = 'Judgment Day/Bankruptcy of Lehman Brothers',pos = 2,col='red')
  
  peakDate <- tail(obj[which.max(obj[,2]),],1)
  abline(v=as.numeric(peakDate[,1]),lty=2)
  abline(h=as.numeric(peakDate[,2]),lty=2)
  text(x = as.numeric(peakDate[,1]),y = as.numeric(peakDate[,2]),
       labels = paste0('Peak. ',JudgmentDay-peakDate[,1],'days before'),pos=2,srt=90,cex=1.2,offset = 1)
}
```
