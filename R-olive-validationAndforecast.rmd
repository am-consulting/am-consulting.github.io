...
funPlot02<-function(){
  ...
  legend("topleft", legend = colnames(dataSet01)[2], col = "blue", lty = 1,pch =1)
}
```
...
funValidationPlot<-function(){
funPlot02()
lines(dataSet01[,1],dataSet01[,3],type="o",col="black")
...
# validation
for(durationType in 1:length(duration)){
...
cat('<h3>Pattern',durationType,':',subtitle,'の為替レートによる予測検証</h3>')
funPlot01()
fundataSetValidation()
cat('<hr>')
cat('ARFIMAによる予測と為替レートの検証')
funValidationPlot()
cat('<hr>')
}
```
...
subtitle <- paste(x[1, 1], " - ", x[nrow(x), 1])
cat('<h3>Pattern',durationType,':',subtitle,'の為替レートによる予測</h3>')
funPlot01()
fundataSetForecast()
dataSet01[,-1]<-round(dataSet01[,-1],3)
cat('<hr>')
