---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');htmlName<-'JGBheldByBOJ';rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-',htmlName,'.html')) })
...
bufMD <- 
  cbind(by(tmp[, 3], JGB, sum))
bufMD <-
  data.frame(`銘柄`=row.names(bufMD),
             bufMD,
             stringsAsFactors = F,check.names = F,row.names = NULL)
colnames(bufMD)[2] <- '残高計(兆円)'
bufMD <-
  bufMD[order(bufMD[,2],decreasing = T),]
bufMD$`構成比(%)` <- 
  round(bufMD[,2]/sum(bufMD[,2])*100,1)
bufMD <-
  rbind(bufMD,c('合計',sum(bufMD[,2]),100))
colnames(bufMD)[2] <- paste0(gsub('.+\\((.+)\\)','\\1',title),colnames(bufMD)[2])
fun_markdownTable(obj = bufMD)
```
...
##################
attach(formdfile)
obj <- cbind(by(formdfile[, 3], JGB, sum))
obj <- obj[order(obj[,1]),,drop=F]
obj <- data.frame(rownames(obj),obj,stringsAsFactors = F,check.names = F,row.names = NULL)
colnames(obj) <- c('国債銘柄','保有残高(兆円)')
par(family = 'Meiryo',mar = c(4,6,5,2),cex.main = 1.2,font.main = 1)
barplot(height = obj[,2],horiz = T,las=2,main = dataTitle,names.arg = obj[,1])
...
```{r warning=F,error=F,message=F,echo=F,results='hide'}
fun_outputMD(tags = '日本銀行, 日本国債, 変動利付債, 物価連動債',
             objDF = obj,
             htmlName = htmlName,
             image1 = 1,
             title = dataTitle,
             tableTitle = '日本銀行が保有する国債残高合計(兆円)',
             tableName = '日本銀行が保有する国債残高合計(兆円)')
fun_updateURLcsv()
```
