...
dataTitle<-'報告省令レートの時系列推移 2016年10月分'
dataSource<-'日本銀行'
...
for(ccc in 2:ncol(dataTS)){ 
  ...
  plot(
    ...
    cex.axis=1.0,cex.lab=1,cex.main=1.0
  )
  lo <- loess(tmp0[, 2]~as.numeric(tmp0[, 1]))
  lines(tmp0[, 1] , predict(lo) , col='red', lwd=1)
  abline(lm(tmp0[, 2]~as.numeric(tmp0[, 1])), col='red', lty=2)
  axis.Date(side=1,at=tmp0[,1],format=dateFormat,padj=0.0,cex.axis=1.0)
}
}
```
...
funPlotBoxplotAmcharts <- function(obj, bigTitle = '') {
  ...
  amBoxplot(obj[,-1], horiz = T,names = colnames(obj)[-1]) %>>%amOptions(theme = "none")
  # c("none", "light", "dark", "patterns", "chalk")
}
```
...
tableRange <- 6
needCompare <- 0
dataSetBP<- dataSet[,1:2]
```

```{r,warning=F,error=F,message=F,results='asis',echo=F} 
dateRange<-paste0(format(first(dataSetBP[,1]),dateFormat),'~',format(last(dataSetBP[,1]),dateFormat))
funPlotBoxplotAmcharts(obj = dataSetBP,bigTitle = paste(dataTitle,dateRange,'. Powered By https://www.amcharts.com/'))
```
<hr>  
```{r,warning=F,error=F,message=F,results='asis',echo=F}
if(plotChunk01==1){
...
if(needCompare==1) {
  cat('<hr>')
  cat('<h4>民主党政権時と第二次安倍政権発足以降の比較</h4>')
}
```
