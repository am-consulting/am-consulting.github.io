...
needAdd<-1
pp<-1
dataTitle<-'消費税を除く国内企業物価指数 2016年7月分'
dataSource<-'日本銀行'
...
funTimeSeries(dataTS=dataSet ,lineType='l', rowN=4 , bigTitle='原時系列データ')
```
<hr>
```{r,warning=F,error=F,message=F,results='asis',echo=F,fig.height=14,fig.width=14}
funTimeSeries(dataTS=dataSet12P ,lineType='h', rowN=4 , bigTitle='前年同月比(%)')
```
<hr>
...
<hr>
```{r,warning=F,error=F,message=F,results='asis',echo=F}
funTableReshape(obj=dataSet12P)
appendTitle<-'前年同月比(%)'
funDataTable(obj = dataSetDT, append = appendTitle)
```