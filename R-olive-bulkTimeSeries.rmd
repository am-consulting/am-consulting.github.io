...
dataTitle<-'石油消費量(1000バレル/日) OECDメンバー 2016年7月公表分'
...
for(ppp in 1:ceiling((ncol(dataSet)-1)/plotBorder)){
  ...
  funTimeSeries(dataTS=plotData[[ppp]] ,lineType='l', rowN=3 , bigTitle=dataTitle)
}
}  
```
<hr>
...
for(ppp in 1:ceiling((ncol(dataSet01P)-1)/plotBorder)){
  ...
  funTimeSeries(dataTS=plotData[[ppp]] ,lineType='h', rowN=3 , bigTitle='前月比(%)')
}
}  
```
