...
fun_dygraphPlot(tsData = obj,shadePattern = 3,drawPoints = c(T,F),
                width = 300,fillGraph = c(F,F),pointSize = c(2,0))
cat('\n\n')
```

```{r warning=F,error=F,message=F,echo=F,results='asis'}
shadePattern = 3;drawPoints = c(T);width = 300;fillGraph = c(T);pointSize = c(2)
cat('# xヶ月(期)前との比較\n\n')
...
cat('## x=',lag[iii],'\n\n')
tmp <- ratioDF[[iii]]
fun_dygraphPlot(tsData = tmp,shadePattern = shadePattern,drawPoints = drawPoints,
                width = width,fillGraph = fillGraph,pointSize = pointSize)
cat('\n<hr>\n')
```
