...
if(htmlName=='00014'){
  ...
  buf <- 
    buf[buf[,1] < as.Date(paste0(year(Sys.Date()),'-',month(Sys.Date()),'-1')),]
  obj <-
...
if(htmlName=='00027'){
  ...
  eval(parse(text = script))
  obj <- PERPBR[,c(1,2)]
  obj[,1] <- as.Date(paste0(year(obj[,1]),'-',month(obj[,1]),'-1'))
...
if(htmlName=='00029'){
  ...
  # http://www.e-stat.go.jp/SG1/estat/OtherList.do?bid=000001025429&cycode=1
  # http://www.e-stat.go.jp/SG1/estat/List.do?lid=000001184644
  ...
  script <-
    RCurl::getURL(
      ...
      ssl.verifypeer = F)
  eval(parse(text = script)) 
  targetURL <- 'http://www.e-stat.go.jp/SG1/estat/Xlsdl.do?sinfid=000026271631'
  fun_MonthlyLabourSurveyIndices(fileID = targetURL)
  fileNumber <- regmatches(targetURL, gregexpr('[0-9]{12}', targetURL, fixed = F))
...
if(htmlName=='00030'){
  importData <- c('importPPPData.r')
  script <-
    RCurl::getURL(paste0(baseURL,importData[1]),ssl.verifypeer = FALSE)
  eval(parse(text = script))
  obj <- PPPDataUSDJPY[,c(1,3)]
  colnames(obj)[2] <- paste0('Purchasing Power Parity:USDJPY:',colnames(obj)[2])
  obj <- subset(obj,as.Date('2007-1-1') <= obj[,1])
  dataSource <- "公益財団法人国際通貨研究所"
  dateFormat <- '%Y-%m'
  diffType <- 1
  rollingN <- 12
}
if(htmlName=='00031'){
  ...
  fileURL <- 'http://www5.cao.go.jp/keizai3/getsurei/1712gap.xls'
  fileName <- gsub('.+/([^/]+\\.xls)','\\1',fileURL)
  pathOutput <- 
  ...
  colnames(buf2)[2] <- paste0('内閣府版:',colnames(buf2)[2])
  obj <- buf2
  dataSource <- "内閣府"
  dateFormat <- '%Y-%m'
  diffType <- 2
...
if(htmlName=='00032'){
  script <-
    RCurl::getURL(
      url = "https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/importDevelopmentsInRealExportsandRealImports-2017.r",
      ssl.verifypeer = FALSE)
  eval(parse(text = script))
  obj <- RealExportsAndRealImports01[,c(1,2)]
  dataSource <- "日本銀行"
  dateFormat <- '%Y-%m'
  diffType <- 1
  rollingN <- 12
...
if(htmlName=='00033'){
  script <-
    RCurl::getURL(
      ...
      ssl.verifypeer = FALSE)
  eval(parse(text = script))
  obj <- RealExportsAndRealImports01[,c(1,3)]
  dataSource <- "日本銀行"
  dateFormat <- '%Y-%m'
  diffType <- 1
  rollingN <- 12
...
if(htmlName=='00034'){
  script <-
    RCurl::getURL(
      url = "https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/importServicesProducerPriceIndex-2017.r",
      ssl.verifypeer = F)
  eval(parse(text = script))
  obj <- na.omit(ServicesProducerPriceIndex[,c(1,2)])
  dataSource <- "日本銀行"
  dateFormat <- '%Y-%m'
  diffType <- 2
  rollingN <- 12
...
if(htmlName=='00035'){
  script <-
    RCurl::getURL(
      ...
      ssl.verifypeer = F)
  eval(parse(text = script))
  obj <- na.omit(ServicesProducerPriceIndex[,c(1,3)])
  dataSource <- "日本銀行"
  dateFormat <- '%Y-%m'
  diffType <- 2
  rollingN <- 12
...
library(plotly)
cat('# Histogram\n\n')
p <- plotly::plot_ly(x = ~obj[,2],type = "histogram",
                     marker = list(color = 'rgb(158,202,225)',
                                   line = list(color = 'rgb(8,48,107)',
                                               width = 1.0))) %>%
  config(displayModeBar = FALSE) %>%
  layout(title = paste0('Histogram of ',colnames(obj)[2]),
         xaxis = list(title = ""),
         yaxis = list(title = ""))
p
cat('\n<hr>\n')
...
fun_dygraphPlot(tsData = objWithfFit,shadePattern = 3,drawPoints = c(T,F),
                ...
                mainTitle = colnames(objWithfFit)[2],strawBroom = 0)
cat('\n<hr>\n')
```

```{r warning=F,error=F,message=F,echo=F,results='asis',out.width='100%'}
cat('- Straw Broom Chart\n\n')
fun_dygraphPlot(tsData = obj,shadePattern = 3,drawPoints = c(T,F),
                width = legendWidth,fillGraph = c(F,F),pointSize = c(2,0),show = 'onmouseover',
                mainTitle = colnames(objWithfFit)[2],strawBroom = 1)
cat('\n<hr>\n')
```

```{r warning=F,error=F,message=F,echo=F,results='asis',out.width='100%'}
fun_variance <- function(obj,objCol = 2,rangeN = 12){
  varDF <- data.frame()
  cnt <- 0
  for(rrr in rangeN:nrow(obj)){
    cnt <- cnt + 1
    tmp <- obj[(rrr-rangeN+1):rrr,]
    varDF[cnt,1] <- as.character(tail(tmp[,1],1))
    varDF[cnt,2] <- var(tmp[,objCol])
  }
  varDF[,1] <- as.Date(varDF[,1])
  colnames(varDF) <- c('Date',paste0(colnames(obj)[objCol],':Variance'))
  mergeData <- merge(obj,varDF,all=T)
  return(mergeData)
}
cat('# Variance\n\n')
varResult <- fun_variance(obj = obj,objCol = 2,rangeN = rollingN)
tsData <- na.omit(varResult[,c(1,3)])
fun_dygraphPlot(tsData = tsData,shadePattern = 3,drawPoints = c(T,F),
                width = legendWidth,fillGraph = c(T,F),pointSize = c(2,0),show = 'onmouseover',
                mainTitle = paste0(colnames(tsData)[2],':n=',rollingN),strawBroom = 0,quantiles = 2)
cat('\n<hr>\n')
```
