...
omitType <- 1
dataTitle <- 'Baker Hughes:North America Rig Count Oil ans Gas / 北米リグカウント 石油とガス - 2016年10月14日'
...
plot(
  ...
  main = paste(mainTitle,'\n', paste0(format(range(dataSet[,1]) ,dateFormat),collapse = '~'),'\nSource:',dataSource),
  cex.axis = 1,
  cex.lab = 1,
  cex.main = 1,
  lwd = obj01lwd
...
# hline part
h01 <- summary(dataSet[,obj01])[2]
h03 <- summary(dataSet[,obj01])[5]
abline(h = h01, col = lineColor[1], lty=2)
text(par('usr')[1], h01, labels = '\n1st Qu.', pos = 4, col = lineColor[1])
abline(h = h03, col = lineColor[1], lty=2)
text(par('usr')[1], h03, labels = '\n3rd Qu.', pos = 4, col = lineColor[1])
# hline part
if(needloess01==1){
...
# hline part
h01 <- summary(dataSet[,obj02])[2]
h03 <- summary(dataSet[,obj02])[5]
abline(h = h01, col = lineColor[2], lty=2)
text(par('usr')[1], h01, labels = '\n1st Qu.', pos = 4, col = lineColor[2])
abline(h = h03, col = lineColor[2], lty=2)
text(par('usr')[1], h03, labels = '\n3rd Qu.', pos = 4, col = lineColor[2])
# hline part
...
# rect part
rectDateL <- as.numeric(as.Date('2009-09-1'))
rectDateR <- as.numeric(as.Date('2012-12-1'))
rectText <- '民主党政権'
...
# obj02 <- 3
attach(dataSetS)
by(dataSetS[obj01:obj02],Group,summary)
```
<hr>
####政権開始時と政権最終(又は現職)時比較
```{r,warning=F,error=F,message=F,results='asis',echo=F}
cat(paste0(unique(dataSetS$Group)[1],' - 表最下段は政権最終(又は現職)月÷政権開始月'))
buf000 <- subset(dataSetS,dataSetS$Group==unique(dataSetS$Group)[1])
result <- data.frame(rbind(tail(buf000[,1:3],1),head(buf000[,1:3],1),
                     cbind(Date=tail(buf000[,1],1), tail(buf000[,2:3],1)/head(buf000[,2:3],1))),
                     check.names = F,row.names = NULL)
result[,1]<-format(result[,1],dateFormat)
knitr::kable(result)
cat(paste0(unique(dataSetS$Group)[2],' - 表最下段は政権最終(又は現職)月÷政権開始月'))
buf000 <- subset(dataSetS,dataSetS$Group==unique(dataSetS$Group)[2])
result <- data.frame(rbind(tail(buf000[,1:3],1),head(buf000[,1:3],1),
                     cbind(Date=tail(buf000[,1],1), tail(buf000[,2:3],1)/head(buf000[,2:3],1))),
                     check.names = F,row.names = NULL)
result[,1]<-format(result[,1],dateFormat)
