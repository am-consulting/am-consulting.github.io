...
dateFormat <- '%Y-%m'
dataTitle <- paste0('全産業活動指数(季節調整値):',
                    paste0(format(range(allIndustryActivity[,1]),dateFormat),collapse = '~'))
dataSource <- '経済産業省'
...
n1 <- 1
fun_generateKnit(objDF = allIndustryActivity[,c(1,2)], plotNum = n1, 
                 colors = 'blue', dygraphTitle = colnames(allIndustryActivity)[2])
```

`r paste(knit(text = c(get(paste0('knit_expanded',n1)))), collapse = '\n')`

#### 季節調整値:直近`r tailN <- 9;tailN`ヶ月

```{r warning=F, error=F, message=F, echo=F}
knitr::kable(fun_transposeDataFrame(obj = allIndustryActivity,dateFormat = dateFormat,
                                    tailN = tailN),row.names = F)
```
...
```{r warning=F, error=F, message=F, echo=F}
AIAMoM <- 
  data.frame(Date = tail(allIndustryActivity[,1],-1),
             round(diff(as.matrix(allIndustryActivity[,-1]))/head(allIndustryActivity[,-1],-1)*100,1),
             check.names = F,stringsAsFactors = F)
knitr::kable(fun_transposeDataFrame(obj = AIAMoM,dateFormat = dateFormat,
                                    tailN = tailN),row.names = F)
```
