---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-FredPopularTS.html')) })
title : "FRED Popular Time Series"
author: "アセット･マネジメント･コンサルティング株式会社 http://am-consulting.co.jp"
date: "`r format(Sys.time(), '%Y-%m-%d %H:%M:%S')`"
output: 
  html_document:
    fig_height: 4
    fig_width: 10
    keep_md: no
    md_extensions: -ascii_identifiers 
---

<link href="http://knowledgevault.saecanet.com/components/rmarkdown-html.css" rel="stylesheet">
<script src="http://knowledgevault.saecanet.com/components/JavascriptCode.js"></script>
<style>.main-container {max-width : 1000px;}</style>

- アセット･マネジメント･コンサルティング株式会社 http://am-consulting.co.jp
- SaECaNet http://saecanet.com/
- Olive http://olive.saecanet.com/
- twitter https://twitter.com/AMC2_Japan
- 本ページへのリンクを共有:<a href="https://twitter.com/share" class="twitter-share-button" data-via="AMC2_Japan" data-size="large">Tweet</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>

```{r set-options, echo=F, cache=F}
options(width = 1000)
```

```{r warning = F, error = F, message = F, echo = F}
library(RCurl)
fileName <- 'functionList.csv'
functionList <-
  read.csv(
    file = paste0('https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/csv/',
                  fileName),
    header = F,
    as.is = T)
for(iii in 1:nrow(functionList)){
  script <- 
    getURL(paste0("https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/", 
                  functionList[iii,1]),
           ssl.verifypeer = F)
  eval(parse(text = script))
}
```

```{r warning=F,error=F,message=F,echo=F}
script <-
  getURL(
    "https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/importFREDpopular10.r",
    ssl.verifypeer = FALSE
  )
eval(parse(text = script))
```

```{r warning=F,error=F,message=F,echo=F,results='hold'}
dataTitle <- 
  paste0('FRED Economic Data: 10 Popular Series')
dataSource <- 'Federal Reserve Bank of St. Louis'
windowsFonts(Meiryo = windowsFont("Meiryo"))
options(scipen = 999)
lineColor <- c('green','red')
```

***

- `r paste0("タイトル : ", dataTitle)`
- `r paste0("データ出所 : ", dataSource)`

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 2
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m'
maN <- 6
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 3
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m'
maN <- 6
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 4
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m-%d'
maN <- 20*3
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 5
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m-%d'
maN <- 20*3
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 6
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m'
maN <- 4
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 7
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m'
maN <- 6
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 8
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y'
maN <- 5
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 9
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m-%d'
maN <- 20*3
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 10
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m'
maN <- 4
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='asis'}
n <- 11
obj <- na.omit(popular10[,c(1,n)])
dateFormat <- '%Y-%m-%d'
maN <- 20*3
fun_MovingAverage(obj = obj,n = maN,dateFormat = dateFormat)
fun_generateKnit(objDF = MovingAverageDF[,-3],plotNum = n,
                 dygraphTitle = colnames(obj)[2],group = 0,colors = lineColor)
eval(parse(text = paste0('dygraphPlot', n, ' %>% ', eventE_dygraph_point)))
fun_transposeDataFrame(obj = obj,tailN = 3,dateFormat = dateFormat)
fun_markdownTable(obj = tDF)
```

```{r warning=F,error=F,message=F,echo=F,results='hide'}
objColumn <- c(2,3,7)
obj <- na.omit(popular10[,c(1,objColumn)])
fun_transposeDataFrame(obj = obj,tailN = 1,dateFormat = '%Y-%m')
fun_markdownTable(obj = tDF)

objColumn <- c(4,5,9,11)
obj <- na.omit(popular10[,c(1,objColumn)])
fun_transposeDataFrame(obj = obj,tailN = 1,dateFormat = '%Y-%m-%d')
fun_markdownTable(obj = tDF)

objColumn <- c(6,10)
obj <- na.omit(popular10[,c(1,objColumn)])
fun_transposeDataFrame(obj = obj,tailN = 1,dateFormat = '%Y-%m')
fun_markdownTable(obj = tDF)

objColumn <- c(8)
obj <- na.omit(popular10[,c(1,objColumn)])
fun_transposeDataFrame(obj = obj,tailN = 1,dateFormat = '%Y')
fun_markdownTable(obj = tDF)
```

***
<script>MakeNegative();</script>
<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-3077339-5', 'auto');
  ga('send', 'pageview');

</script>
