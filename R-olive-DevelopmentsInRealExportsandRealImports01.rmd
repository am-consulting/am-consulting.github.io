...
dateFormat <- '%Y年%m月'
dataTitle <- paste0('実質輸出入の動向 - 実質輸出、実質輸入 - ',format(tail(developmentsInRealExportsAndRealImports[,1],1),dateFormat))
dataSource <- '日本銀行'
...
tmp <- developmentsInRealExportsAndRealImports[,c(1,2,3)]
```
...
```{r warning=F, error=F, message=F, echo=F}
knitr::kable(fun_transposeDataFrame(obj = tmp,dateFormat = dateFormat,colName = '項目'),row.names = F)
```
...
```{r warning=F, error=F, message=F, echo=F}
tmpDiff <- 
  data.frame(Date = tail(tmp[,1],-1),round(diff(as.matrix(tmp[,-1]))/head(tmp[,-1],-1)*100,1),
