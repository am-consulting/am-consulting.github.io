...
fun_generateKnit(objDF = timeSeries,
                 plotNum = n,
                 group = 0,
                 dygraphTitle = paste0(colnames(timeSeries)[-1],collapse = ' × '))
...
resultScale[,-1] <-
  apply(resultScale[,-1],2,function(x)x-head(x,1))
...
```{r warning=F,error=F,message=F,echo=F,results='hide'}
mdTable <- 
  na.omit(dataSet[,c(1,grep('japan|united states|euro area',colnames(dataSet),ignore.case = T))])
fun_makeDiffRatioTable(obj = mdTable,lag = 1,roundDigits = 2)
fun_transposeDataFrame(obj = diffRatioDF,tailN = 6)
