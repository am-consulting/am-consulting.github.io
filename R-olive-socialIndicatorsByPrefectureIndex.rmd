...
library(rvest)
url <- 
  'http://www.e-stat.go.jp/SG1/estat/GL08020103.do?_toGL08020103_&tclassID=000001083999&cycleCode=0&requestSender=estat'
htmlMarkup <- 
...
for(fff in 1:length(linkLists)){
  download.file(url = paste0('http://www.e-stat.go.jp/SG1/estat/', linkLists[fff]), 
                destfile = paste0(letters[fff],'Index.xls'), 
                mode = "wb")
}
```

```{r warning=F, error=F, message=F, echo=F, results='hold'}
dataTitle <- '社会生活統計指標-都道府県の指標:2017年'
dataSource <- '総務省'
