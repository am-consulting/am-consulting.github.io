...
```{r}
library(XLConnect);library(Nippon);library(lubridate);library(quantmod)
cntList <- 0
dataDF <- list()
title <- vector()
for(iii in seq(length(indicators))){
  ...
  if(1<length(unique(day(tmp1[,1])))){
    ...
    buf[,1] <- as.Date(buf[,1])
    buf[,2] <- round(buf[,2],4)
    colnames(buf) <- c('Date',paste0(title[iii],':Average in the Month'))
    buf <- head(buf,-1)
    obj <- buf
  }else{
    obj <- tmp1
...
colnames(csvData) <- 
  c('Index','Before<br>TRUMP','After<br>TRUMP','Comparison','','Unit','Latest month(or period)')
