...
```{r warning=F, error=F, message=F, echo=F, results='hold'}
fun_makeDiffTable(mergeData,lag = 1)
...
```{r warning=F,error=F,message=F,echo=F,results='asis'}
fun_dataTable(obj = tail(ProducerPriceIndex,12*4),dateFormat = dateFormat)
```

```{r warning=F,error=F,message=F,echo=F,results='hide'}
fun_transposeDataFrame(obj = ProducerPriceIndex,dateFormat = dateFormat,tailN = 3)
