...
title : 'Developments in Real Exports and Real Imports in Japan'
...
dataTitle <- '実質輸出入の動向 - 対アメリカ、EU、中国'
dataSource <- '日本銀行'
...
head(tmp)
objCC <- seq(1,ncol(tmp))
objCC <- seq(1,4)
#objCC <- c(1,5,6,7)
dataSet <- na.omit(tmp[,objCC])
if(length(dataSet[,-1][dataSet[,-1]>=0])!=0 & length(dataSet[,-1][dataSet[,-1]<0])!=0){barPlot <- 1}
if(length(unique(month(dataSet[,1]))) == 4){quarterly <- 1}
```
...
fun_dygraph <- function(obj = dataSet, mainTitle = ""){
  ...
  dygraphPlot <- 
    ...
    if(barPlot == 1){dygraphPlot <- dygraphPlot %>% dyMultiColumn()}
...
bufDF <- dataSet[c(nrow(dataSet),nrow(dataSet)-1,nrow(dataSet)-12),]
if(quarterly==1){
bufDF <- dataSet[c(nrow(dataSet),nrow(dataSet)-1,nrow(dataSet)-4),]
}
bufDF[,1] <- format(bufDF[,1],dateFormat)
