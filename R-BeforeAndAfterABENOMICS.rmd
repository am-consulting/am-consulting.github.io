...
functionList <- 
  read.csv(
    file = paste0(baseURL01,'csv/',fileName),
    header = F,
    as.is = T)
...
eval(parse(text = script))
cntList <- cntList + 1
dataDF[[cntList]] <- na.omit(MoneyStock[,c(1,10,11,13)])
...
csvData <- csvData[,c(1,2,3,4,6,7,5)]
colnames(csvData) <- 
  c('No.','指標','B.A:2012年12月(月期)','A.A:直近','比較','比較単位','直近月(月期)')
```
