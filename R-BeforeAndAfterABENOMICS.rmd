...
setwd(pathOutput)
targetURL <- 'http://www.e-stat.go.jp/SG1/estat/Xlsdl.do?sinfid=000031616607'
...
print(tail(dataDF[[cntList]]))
obj <- gdp5[,c(1,2)]
colnames(obj)[2] <- gsub('支出側','支出側,四半期',colnames(obj)[2])
cntList <- cntList + 1
dataDF[[cntList]] <- na.omit(obj)
print(tail(dataDF[[cntList]]))
# 財務省貿易統計
...
print(tail(dataDF[[cntList]]))
# 預金貸出動向
importData <- 'importPrincipalFiguresOfFinancialInstitutions-2017.r'
script <-
  RCurl::getURL(paste0(baseURL01,importData),ssl.verifypeer = FALSE)
eval(parse(text = script))
obj <- PrincipalFiguresOfFinancialInstitutions[,c(1,2)]
colnames(obj) <- gsub('\\([a-z]+\\)','',colnames(obj))
