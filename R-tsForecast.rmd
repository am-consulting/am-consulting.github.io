...
for(iii in 1:1){
  ...
  print(g)
  g <- ggplot(data = residualsData)
  g <- g + geom_line(mapping = aes_string(x = 'Date',y = 'Residual'))
  ...
  g <- g + theme(axis.text.x = element_text(angle = 90,family = "Meiryo",size = 12,vjust = 0.5)) 
  g <- g + theme(axis.text.y = element_text(angle = 0,family = "Meiryo",size = 16))
  zeroLine <- data.frame(x = c(head(residualsData[,1],1),tail(residualsData[,1],1)),y = 0)
  g <- g + geom_line(data = zeroLine,aes(x = x, y = y),colour='red')
  shapiro.test(residualsData[,2])
  acf(as.vector(residualsData[,2]))
  print(g)
}
```
