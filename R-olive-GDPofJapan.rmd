...
for(iii in 1:15){
  ...
  fun_markdownTable(obj = fun_transposeDataFrame(obj = GDPdata,dateFormat = dateFormat,tailN = 3))
  cat('<hr>')
}
```
...
##################
obj <- merge(gdp3[,c(1,2)],gdp4[,c(1,2)],by='Date')
fun_plotTimeSeries(obj = obj,chartType = 1,typeL = 'o',typeR = 'o',
                   dateFormat = dateFormat,dataSource = dataSource,needLefLab = 0,fitLcolor = 0,fitRcolor = 0,mar = c(4,4,5,1))
abline(h=0,lwd=2)
dev.off()
```
...
fun_outputMD(tags = '国内総生産',
             objDF = fun_transposeDataFrame(obj = obj,dateFormat = dateFormat,tailN = 4),
             htmlName = htmlName,
             image = 1,
             dataTitle = dataTitle,
             tableTitle = '直近4期',
