...
tmp <- developmentsInRealExportsAndRealImports[,c(1,5,6,7)]
```
...
fun_plot_dygraph <- function(obj = tmp){
  buf <- na.omit(obj)  
  # fun_dygraph(obj = buf, mainTitle = paste0(colnames(buf)[-1], collapse =  ' × '))
  fun_dygraph(obj = buf, mainTitle = dataTitle)
...
```{r warning=F, error=F, message=F, echo=F}
eval(parse(text = paste0('dygraphPlot %>% ',ConsumptionTax)))
```

***

```{r warning=F, error=F, message=F, echo=F}
eval(parse(text = paste0('dygraphPlot %>% ',primeMinisterOfJapan_dygraph)))
```

***

```{r warning=F, error=F, message=F, echo=F}
eval(parse(text = paste0('dygraphPlot %>% ',boj_dygraph)))
```

***

```{r warning=F, error=F, message=F, echo=F}
eval(parse(text = paste0('dygraphPlot %>%  ',event_dygraph_point,' %>%  ',event_dygraph_range)))
```
