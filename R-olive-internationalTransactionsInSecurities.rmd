...
tableStat[,3] <- format(tableStat[,3], big.mark=",", scientific=F)
colnames(tableStat) <- c('ID','年･半期･四半期','億円')
DT::datatable(
  tableStat,
  options = list(
  ...
  caption = paste0('対内-株式･投資ファンド持分-ネット:', head(origData[,2],1), ' ~ ', tail(origData[,2],1))
)
```
<hr>
