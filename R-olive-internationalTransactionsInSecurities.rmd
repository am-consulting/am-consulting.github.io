...
td.AMCCTDstyle01 {text-align : right;font-style: italic;}
td.AMCCTDstyle02 {color : blue;text-align : right;font-style: italic;font-weight: bold;}
</style>
<script>
...
DT::datatable(
  ...
  options = list(paging = T, autoWidth = F, info = T, lengthChange = T, ordering = T,
    ...
    columnDefs = list(list(className = "AMCCTDstyle01", targets = 0),
                      list(className = "AMCCTDstyle02", targets = (grep('ネット',colnames(obj))-1)))),
  rownames = F, caption = caption, class = 'display compact', escape = F
)  
}
# fun_datatabls(obj = origData[,c(1,2,5,8,13,16,19,24)])
fun_datatabls(obj = origData[,-grep('year|month|h|q',colnames(origData),ignore.case = T)])
```
...
DT::datatable(
  ...
  rownames = F,   class = 'display compact',
...
# fun_datatabls(obj = buf,orderDirection = 'asc')
```
