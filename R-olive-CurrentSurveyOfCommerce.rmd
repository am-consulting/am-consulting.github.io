...
##################
obj1 <- departmentStore[,c(1,2)]
colnames(obj1)[2] <- paste0('百貨店:',colnames(obj1)[2],'(億円)')
obj2 <- superMarket[,c(1,2)]
colnames(obj2)[2] <- paste0('スーパー:',colnames(obj2)[2],'(億円)')
mergeData <- merge(obj1,obj2,by='Date')
fun_plotTimeSeries(obj = mergeData,chartType = 1,
                   dateFormat = dateFormat,dataSource = dataSource,
                   needLefLab = 0,fitlwdL = 3,fitlwdR = 3,mar = c(4,4,5,1),dataTitle = '商業動態統計調査')
dev.off()
```
