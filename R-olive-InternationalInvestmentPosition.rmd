---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');htmlName<-'InternationalInvestmentPosition';rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-',htmlName,'.html')) })

title : 'Trade Statistics of Japan:Preliminary Report'
author: 'アセット･マネジメント･コンサルティング株式会社 http://am-consulting.co.jp'
date: "`r format(Sys.time(), '%Y-%m-%d %H:%M:%S')`"
output: 
  html_document:
    fig_height: 4
    fig_width: 10
    keep_md: no
    md_extensions: -ascii_identifiers 
---
<link href="http://knowledgevault.saecanet.com/components/rmarkdown-html.css" rel="stylesheet">
<style>.main-container {max-width : 1000px;}</style>
<script src="http://knowledgevault.saecanet.com/components/JavascriptCode.js"></script>

- アセット･マネジメント･コンサルティング株式会社 http://am-consulting.co.jp
- SaECaNet http://saecanet.com/
- Olive http://olive.saecanet.com/
- twitter https://twitter.com/AMC2_Japan
- 本ページへのリンクを共有:<a href="https://twitter.com/share" class="twitter-share-button" data-via="AMC2_Japan" data-size="large">Tweet</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>

```{r set-options, echo=F, cache=F}
options(width = 1000)
```

```{r warning = F, error = F, message = F, echo = F}
library(RCurl)
fileName <- 'functionList.csv'
functionList <-
  read.csv(
    file = paste0('https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/csv/',
                  fileName),
    header = F,
    as.is = T)
for(iii in 1:nrow(functionList)){
  script <- 
    getURL(paste0("https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/", 
                  functionList[iii,1]),
           ssl.verifypeer = F)
  eval(parse(text = script))
}
```

```{r warning=F,error=F,message=F,echo=F,results='hide'}
script <-
  getURL(
    "https://raw.githubusercontent.com/am-consulting/am-consulting.github.io/master/importInternationalInvestmentPosition.r",
    ssl.verifypeer = F
  )
eval(parse(text = script))
```

```{r warning=F, error=F, message=F, echo=F, results='hold'}
dateFormat <- '%Y年'
obj <- InternationalInvestmentPosition[,c(1,2,8,13)]
fun_latestResult(obj = obj,objColumn = c(2,3,4),dateFormat = dateFormat)
dataTitle <- 
  paste0(sheetTtile,':',latestResult)
dataSource <- 
  '財務省'
windowsFonts(Meiryo = windowsFont('Meiryo'))
```

***

- `r paste0("タイトル : ", dataTitle)`
- `r paste0("データ出所 : ", dataSource)`
- Source http://www.mof.go.jp/international_policy/reference/iip/data.htm

***

```{r warning=F, error=F, message=F, echo=F, results='asis'}
n <- 1
obj <- InternationalInvestmentPosition[,c(1,2,8)]
fun_generateKnit(objDF = obj,
                 plotNum = n,group = 0,dygraphTitle = paste0(colnames(obj)[-1],collapse = '×'),
                 colors = c('blue','red'))
eval(parse(text = paste0('dygraphPlot', n)))
cat('<hr>')
n <- 2
obj <- InternationalInvestmentPosition[,c(1,13)]
fun_generateKnit(objDF = obj,
                 plotNum = n,group = 0,dygraphTitle = paste0(colnames(obj)[-1],collapse = '×'),
                 barPlot = 1,quantiles = 2)
eval(parse(text = paste0('dygraphPlot', n)))
cat('<hr>')
obj <- InternationalInvestmentPosition[,c(1,2,8,13)]
fun_dataTable(obj = obj,dateFormat = dateFormat)
```

***

```{r warning=F, error=F, message=F, echo=F, results='asis'}
fun_transposeDataFrame(obj = InternationalInvestmentPosition,tailN = 6,dateFormat = dateFormat)
fun_markdownTable(tDF)
```

***

```{r warning=F,error=F,message=F,echo=F,results='hide'}
fun_makeDiffRatioTable(obj = InternationalInvestmentPosition,lag = 1)
fun_transposeDataFrame(obj = diffRatioDF,dateFormat = dateFormat,tailN = 6)
fun_outputMD(tags = '財務省,本邦対外資産負債残高',
             objDF = tDF,
             htmlName = htmlName,
             tableTitle = paste0('本邦対外資産負債残高前年比(%)'),
             tableName = '本邦対外資産負債残高前年比(%)')
```

<script>MakeNegative();</script>
<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-3077339-5', 'auto');
  ga('send', 'pageview');

</script>
