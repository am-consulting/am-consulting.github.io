...
dataTitle <- '石油統計月報 原油輸入数量と構成比 2016年6月'
dataTitle0 <- ''
dataSource <- '資源エネルギー庁'
dataSource0 <- ''
...
funDataTable <- function() {
  DT::datatable(
    ...
    rownames = F,
    caption = paste(dataTitle, dataTitle0, dataSource0,'. Source:' ,dataSource),
...
barplot(
  ...
  ylab = ylabText,
  main=paste(dataTitle,dataTitle0,'\n',colnames(dataSet)[objY1],'\nSource:',dataSource),
  family = 'Meiryo'
)
...
objX<-1
ablineSteps<-c(500000,1,0.1,10000)
horizType<-'T'
