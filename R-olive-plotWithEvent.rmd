...
dataTitle <- '中国公船による尖閣諸島接近(接続水域入域･領海侵入)状況の統計分析'
...
colnames(tableObj)[-1] <- c('カテゴリ:月', paste0(colnames(dataSet)[obj01],'(カテゴリ合計)'))
funDataTable()
```
<hr>
```{r,warning=F,error=F,message=F,echo=F,fig.height=4,fig.width=11} 
...
tableObj <-
  data.frame(ID = seq(1, nrow(aggreTable)), aggreTable, check.names = F)
colnames(tableObj)[-1] <- c('カテゴリ:日', paste0(colnames(dataSet)[obj01],'(カテゴリ合計)'))
funDataTable()
```
<hr>
```{r,warning=F,error=F,message=F,echo=F,fig.height=4,fig.width=11} 
...
tableObj <-
  data.frame(ID = seq(1, nrow(aggreTable)), aggreTable, check.names = F)
colnames(tableObj)[-1] <- c('カテゴリ:年', paste0(colnames(dataSet)[obj01],'(カテゴリ合計)'))
funDataTable()
```
<hr>
```{r,warning=F,error=F,message=F,echo=F} 
...
hist(
  ...
  main = paste0(
    ...
    last(statDataSet[, 2]),
    '(',
    last(statDataSet[, 2])-first(statDataSet[, 2]),
    '日間)'
  ),
