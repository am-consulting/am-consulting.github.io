---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');htmlName<-'intrusion';rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-',htmlName,'.html')) })
...
pathToCSV <- 
  paste0("C:/Users/", username, buf000[2,1], 'csv/Intrusion/')
setwd(pathToCSV)
getwd()
intrusionCSVData <- dataSet0
csvfileName <- 'intrusionIntoTerritorialWaters.csv'
write.csv(x = intrusionCSVData,
          file = csvfileName,
          quote = T,
          row.names = F,
          append = F,
...
script <-
  RCurl::getURL(
    ...
    ssl.verifypeer = F)
eval(parse(text = script))
fun_writeCSVtoFolder(objData = dataSet0,
                     dataType = 1,
                     csvFileName = '中国公船等による尖閣諸島周辺の接続水域内入域及び領海侵入隻数')
# csv出力パート
```
...
fun_markdownTable(obj = fun_transposeDataFrame(obj = na.omit(dataSet),
                                               tailN = 5,dateFormat = '%Y年%m月%d日'))
```
...
##################
# obj <- tail(dataSet[,c(1,2)],14)
# par(mar=c(4,6.5,2,1),family='Meiryo',mfrow=c(1,1))
# barplot(height = obj[,2],
#         names.arg = obj[,1],
#         horiz = T,
#         las = 1,
#         col = '#dc143c',
#         main = '中国公船等による尖閣諸島周辺の接続水域内入域数(隻):直近14日分')
# ggplot
obj <- dataSet[,c(1,2)]
dateRange <- paste0(range(obj[,1]),collapse = '~')
g <- ggplot(data = obj,aes(x = obj[,1],y = obj[,2]))
g <- g + geom_bar(stat = 'identity',col = '#4169e1') + geom_point(col = '#4169e1',size = 1)
g <- g + ggtitle(label = paste0('中国公船等による尖閣諸島周辺の接続水域内入域数(隻)\n',
                                dateRange,'\nSource:海上保安庁'))
g <- g + xlab(label = '')
...
```{r warning=F,error=F,message=F,echo=F,results='hide'}
fun_outputMD(tags = '中国,海上保安庁,接続水域入域,領海侵入,尖閣',
             objDF = fun_transposeDataFrame(obj = dataSet,tailN = 5),
             htmlName = htmlName,
             image1 = 1,
             title = dataTitle,
             tableTitle = '直近5日分',
             tableName = '中国公船等による尖閣諸島周辺の接続水域内入域数')
fun_updateURLcsv()
```
