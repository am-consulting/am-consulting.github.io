...
```{r warning = F,error = F,message = F,echo = F,results = 'asis'}
subjectN <- c(11,5,17,25,27)
for(iii in seq(length(subjectN))){
  returnOJB <- fun_extractWEO(objCountry = ISO[isoN],Subject = Subject[subjectN[iii]])
  fun_plot(obj = returnOJB$tsData)
  cat('\n\n<hr>\n\n')
}
```
