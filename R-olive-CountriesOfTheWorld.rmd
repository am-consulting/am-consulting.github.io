---
knit: (function(inputFile, encoding) {username<-Sys.info()['user'];fileName<-'defaultPath.csv';pathToFile <-paste0('C:/Users/', username,'/Desktop/pathToCSV/');setwd(pathToFile);buf0<-read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,fileEncoding = 'utf-8');timeStamp<-format(Sys.time(),'%Y%m%d%H%M%S');htmlName<-'CountriesOfTheWorld';titleCountry<-'Japan';rmarkdown::render(inputFile, encoding = encoding, output_file = paste0('C:/Users/',username,buf0[2,1],'charts/','am-consulting.co.jp-',htmlName,'_',titleCountry,'.html')) })

title : "`r paste0('Countries Of The World:',titleCountry)`"
...
isoN <- grep(titleCountry,unique(WEO$Country),ignore.case = T)
forcastYear <- 2016
cex1 <- 1.1
cex2 <- 2.9
```
...
isoN <- grep(titleCountry,unique(OECD$Country),ignore.case = T)
```
...
for(iii in seq(length(tableTitle))){
  ...
  objRaw <- grep(titleCountry,obj$Country,ignore.case = T)
  if(length(objRaw)!=0){
