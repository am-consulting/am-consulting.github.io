...
fun_plot <- function(obj){
  ...
  plot(obj[,c(1,2)],xlab='',ylab='',
       main = paste0(returnOJB$tsCountry,'\n',
                     colnames(obj)[2],'\n',
                     returnOJB$tsUnit,'\n',
                     if(!is.na(returnOJB$tsScale)){paste0(returnOJB$tsScale,'\n')},
                     dateRange),
       panel.first = grid(nx = NULL,ny = NULL,lty = 2,equilogs = T),
...
for(iii in seq(length(subjectN))){
  returnOJB <- fun_extractWEO(objCountry = ISO[isoN],Subject = SubjectList[subjectN[iii]])
  if(nrow(returnOJB$tsData)!=0){fun_plot(obj = returnOJB$tsData)}
  cat('\n\n<hr>\n\n')
}
```
