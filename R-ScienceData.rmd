...
colnames(seaIceIndex[[iii]])[2] <- paste0(colnames(seaIceIndex[[iii]])[2],':',unit)
print(tail(seaIceIndex[[iii]]))
```
...
fun_ggplot_line <- function(plotData,pngN){
  ...
  pngFile <- paste0('AMCC-science-',pngN,".png")
  png(file = pngFile, width = pngWidth, height = pngHeight)
  dateRange <- paste0(range(plotData$Date),collapse = '~')
  ...
  g <- g + ggtitle(paste('Sea Ice Index Data:',colnames(plotData)[2],'\n',dateRange,
                         '\nSource:',dataSource,
                         "\nUnit Root Test p-value:",tseries::adf.test(plotData[,2])$p.value))
  print(g)
  dev.off()
}
for(iii in 1:3){
  fun_ggplot_line(plotData = seaIceIndex[[iii]],pngN = iii+1)
}
```
