...
library(ggplot2);library(scales);library(ggfortify);library(lubridate)
...
buf0 <-
  read.csv(fileName,header = F,skip = 0,stringsAsFactor = F,check.names = F,
           fileEncoding = 'UTF8')
pathOutputScience <- paste0('C:/Users/',userName,buf0[2,1],'charts/indicator/AMCC-science/')
```

```{r warning=F,error=F,message=F,echo=F,results='hide'}
fun_plotEarthquake <- function(earthQuakeObj,pngN = 1){
setwd(pathOutputScience)
pngFile <- paste0('AMCC-science-Earthquake',pngN,".png")
png(file = pngFile, width = pngWidth, height = pngHeight)
g <- fun_ggplotWorldMap(ratio = 1.15)
g <- g + geom_point(data = earthQuakeObj,aes(x = longitude, y = latitude),colour = "red",
                    alpha = 30/100,size = 4) 
g <- g + geom_text(data = earthQuakeObj,aes(x = longitude,y = latitude,
                                             label = paste0(mag,':',magType)),
                   vjust = 0,hjust = 0,family = "Meiryo",size= 6)
g <- g + ggtitle(paste0('Earthquakes occurred on ',
                        unique(earthQuakeObj$JSTDate),'\nMagnitude over ',borderM,
...
earthQuakeDataOver <- 
  earthQuakeData[borderM<=earthQuakeData$mag,]
earthQuakeDataOver <-
  earthQuakeDataOver[!is.na(earthQuakeDataOver$mag),]
```

```{r warning=F,error=F,message=F,echo=F,results='hide'}
earthObjCol <- c(1,2,3,4,5,6,14,24,25)
dayN <- 1
tmp <- 
  earthQuakeDataOver[earthQuakeDataOver$JSTDate==(Sys.Date()-dayN),earthObjCol]
fun_plotEarthquake(earthQuakeObj = tmp,pngN = dayN)
dayN <- 2
tmp <- 
  earthQuakeDataOver[earthQuakeDataOver$JSTDate==(Sys.Date()-dayN),earthObjCol]
fun_plotEarthquake(earthQuakeObj = tmp,pngN = dayN)
...
for(iii in 1:2){
  ...
  colnames(bufDF0)[2:3] <- 
    paste0('Sea Ice Index:',area,':',colnames(bufDF0)[2:3])
  bufDF <-
    bufDF0[rev(seq(from = tail(bufDF0$Date,1),by = '-1 days',length.out = nrow(bufDF0)))==bufDF0$Date,]
  print(unique(diff(bufDF$Date)))
  seaIceIndex[[iii]] <- bufDF
...
for(iii in 1:3){
  ...
  fun_ggplot_acf(obj = seaIceIndex[[iii]],objCol = 2,pngN = iii,lag.max = 365*10)
}
```
...
eval(parse(text = script))
# month diff check
unique(diff(year(origData_sunspot01$Date)*12+month(origData_sunspot01$Date)))
unique(diff(year(origData_sunspot02$Date)*12+month(origData_sunspot02$Date)))
# month diff check
```
...
for(iii in 1:length(url_CO2)){
  ...
  if(length(grep('_ryo',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Ryōri)/',colnames(tmp)[2])
  }
  if(length(grep('_mnm',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Minami-Tori-shima)/',colnames(tmp)[2])
  }
  if(length(grep('_yon',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Yonaguni-jima)/',colnames(tmp)[2])
  }
  # month diff check
  print(unique(diff(year(tmp$Date)*12+month(tmp$Date))))
  # month diff check
  CO2[[iii]] <- tmp
  print(tail(CO2[[iii]]))
}
...
for(iii in 1:length(sourceURL_MMT)){
  ...
  # month diff check
  print(unique(diff(year(buf4$Date)*12+month(buf4$Date))))
  # month diff check
  MMT[[iii]] <- buf4
  print(tail(MMT[[iii]]))
}
...
for (iii in 1:length(url_SST)) {
  ...
  # month diff check
  print(unique(diff(year(tmp05$Date)*12+month(tmp05$Date))))
  # month diff check
