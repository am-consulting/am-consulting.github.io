...
for(iii in 1:length(url_CO2)){
  ...
  if(length(grep('_ryo',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Ryōri)\n',colnames(tmp)[2])
  }
  if(length(grep('_mnm',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Minami-Tori-shima)\n',colnames(tmp)[2])
  }
  if(length(grep('_yon',url_CO2[iii]))!=0){
    colnames(tmp)[2] <- paste0('CO2 Concentration(ppm):Monthly Average(Yonaguni-jima)\n',colnames(tmp)[2])
  }
  # month diff check
