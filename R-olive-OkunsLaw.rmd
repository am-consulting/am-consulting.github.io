...
obj3 <- 
  obj2[order(obj2[,1]),]
colnames(obj3) <-
  c('Date', paste0(colnames(laborforcesurvey)[17],':四半期平均'))
tail(obj3)
assign('unemploymentRateQuarterlyAverage', obj3, envir = .GlobalEnv)
obj4 <-
  data.frame(tail(unemploymentRateQuarterlyAverage[,1],-1), 
             diff(unemploymentRateQuarterlyAverage[,2]), 
             check.names = F, 
             stringsAsFactors = F)
colnames(obj4) <- 
  colnames(unemploymentRateQuarterlyAverage)
colnames(obj4)[2] <-
  paste0(colnames(obj4)[2],':前期比(PP)')
tail(obj4)
