...
dataTitle <- '建設総合統計 出来高前年同月比(%) 2016年8月分'
dataSource <- '国土交通省'
...
for(iii in 1:length(uniqN)){
  buf <- dataset[which(uniqBuf == uniqN[iii]), c(1,3)]
  data[[iii]] <- buf[,2]
  dataN[[iii]] <- buf[,1]
}
...
<hr>
```{r warning=F, error=F, message=F, echo=F}
funDataTable(obj = dataset, width01 = '10%', width02 = '3%', order01 = 'asc')
```
<hr>
```{r warning=F, error=F, message=F, echo=F}
