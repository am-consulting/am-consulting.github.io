...
tmp <- merge(idName, tmp, by='ID', all = F)
colnames(tmp)
```
...
switch (sss,
  uniqN <- unique(dataset$Group01),
  uniqN <- unique(dataset$Group02),
  uniqN <- unique(dataset$Group03)
)
for(iii in 1:length(uniqN)){
  switch (sss,
    buf <- dataset[which(dataset$Group01 == uniqN[iii]), 2:3],
    buf <- dataset[which(dataset$Group02 == uniqN[iii]), 2:3],
    buf <- dataset[which(dataset$Group03 == uniqN[iii]), 2:3]
  )  
  data[[iii]] <- buf[,2]
  dataN[[iii]] <- buf[,1]
}
...
for(iii in 1:length(uniqN)){
  text(data[[iii]], iii+0.1, labels = dataN[[iii]], srt = 90, adj = 0, cex = 0.9)
}
}
```
<hr>
