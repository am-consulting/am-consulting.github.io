...
```{r}
fun_plot <- function(s = 1,e = 16,n = 10, tailN = 50){
  par(mfrow = c(4,4),oma = c(2,0,2,0),family = 'Meiryo',font.main = 2,mar = c(4,3,4,1),cex.main = 1.1)
  for(iii in s:e){
    ...
    plot(obj[,1],obj[,2],type = 'o',xaxt = 'n',xlab = '',ylab = '',
         pch = 20,cex = 1.3,ylim = c(min(obj[,2:3]),max(obj[,2:3])),
  ...
  mtext(text = paste0('Latest ',tailN,
                      ' days.Source:Oanda,FRED,MOF,CBOE,NIKKEI,IIMA,EIA,JSDA'),
        side = 3,outer = T,col = 'black')
}
```
...
importIndex <- 
  c('importPutCallRatios.r','importSkew.r','importTokyoRepoRate.r',
    'importHenryHubNaturalGasSpotPrice.r','importGMVI.r')
...
dailyChart[[cnt]] <- obj
fun_plot()
fun_plot(s = 17,e = length(dailyChart))
username <- Sys.info()['user']
pathOutput <-
  paste("C:/Users/", username, "/Desktop/R_Data_Write/", sep = "")
setwd(pathOutput)
#NIKKEI , #10YrJGB , #10YrBond , #USDJPY , #VIX, #EURUSD, #Bitcoin, #Gold, #SKEW, #Put, #Call, #TokyoRepoRate, #Henryhub
# 1400 * 900
```
