...
fun_plot <- function(s = 1,e = 16,n = 10,tailN = 50,pngN = 1){
  ...
  mtext(text = paste0('Latest ',tailN,
                      ' days(weeks).Source:Oanda,FRED,MOF,CBOE,NIKKEI,IIMA,EIA,JSDA,S&P'),
...
fun_effectiveFX <- function(){
  ...
  buf0 <- 
    excel.link::xl.read.file(unzip(zipFile), header=F, top.left.cell = "A1", xl.sheet = 1, excel.visible = F)
  buf1 <- buf0
...
fun_plot(s = 65,e = length(dailyChart),pngN = 5)
