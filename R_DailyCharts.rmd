...
print(tail(dailyChart[[cnt]]))
cnt <- cnt + 1
fromYahoo <- c('^TNX','10-Yr U.S.Bond Yield')
for(iii in seq(1,length(fromYahoo),by = 2)){
  obj <- getSymbols(Symbols = fromYahoo[iii],auto.assign = F)
  obj <-
    data.frame(Date = index(obj),
               obj[,grep('close',colnames(obj),ignore.case = T)],
...
fun_MovingAverage(obj = obj0,n= n,dateFormat = dateFormat)
tailN <- 20*2
obj <- tail(MovingAverageDF,tailN)
plot(obj[,1],
     obj[,2],
     type = 'o',
     xaxt = 'n',
     ...
     pch = 20,
     cex = 1.3,
     main = paste0(colnames(obj)[2],'.',tail(obj,1)[,1],':',tail(obj,1)[,2],'\n',
