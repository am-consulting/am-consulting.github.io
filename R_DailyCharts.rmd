...
cnt <- 1
dateFormat <- '%Y%m%d'
fromOANDA <- c('USD/JPY','EUR/USD','BTC/USD','XAU/USD','XAU/XAG','XAU/XPT','BTC/XAU')
...
dailyChart[[cnt]] <- obj
importIndex <- 
  c('importPutCallRatios.r','importSkew.r')
...
plot(obj[,1],
     ...
     main = paste0(colnames(obj)[2],'\n',tail(obj,1)[,1],':',round(tail(obj,1)[,2],3),'\n',
...
mtext(text = paste0('Red Line:Moving Average(Over ',n,' days)'),
      ...
      outer = T,
      col = 'red')
mtext(text = paste0('Latest ',tailN,' days.Source:Oanda,FRED,MOF,CBOE,NIKKEI'),
      side = 3,
      outer = T,
      col = 'black')
username <- Sys.info()['user']
pathOutput <-
  paste("C:/Users/", username, "/Desktop/R_Data_Write/", sep = "")
setwd(pathOutput)
#NIKKEI , #10YrJGB , #10YrBond , #USDJPY , #VIX, #EURUSD, #Bitcoin, #Gold, #SKEW, #Put, #Call
# 1400 * 900
```
